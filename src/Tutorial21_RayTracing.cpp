...
void Tutorial21_RayTracing::CreateProceduralBLAS()
{
    ...
    const HLSL::BoxAttribs Boxes[] = {
        HLSL::BoxAttribs{-2.5f, -2.5, -2.5, 2.5f, 2.5f, 2.5f},
        HLSL::BoxAttribs{-1.0f, -1.0f, -1.0f, 1.0f, 1.0f, 1.0f}};
...
void Tutorial21_RayTracing::UpdateTLAS()
{
    ...
    static constexpr int MaxTotal  = BaseCount + MaximumSmallSpheres;

    bool NeedUpdate = true;

    // Cambiar la declaración de 'Instances' a un vector dinámico
    std::vector<TLASBuildInstanceData> Instances(MaxTotal);  // Inicializar el vector con el tamaño de MaxTotal

    static std::vector<float3> smallSpherePositions;
    if (smallSpherePositions.empty())
    {
        smallSpherePositions.reserve(MaximumSmallSpheres);
        ...
        for (int i = 0; i < MaximumSmallSpheres; ++i)
        {
            float x = distX(rng);
            float z = distZ(rng);
    ...
    if (smallCubePositions.empty()) // Si aún no se han calculado las posiciones
    {
        smallCubePositions.reserve(MaximumSmallCubes); // Reserva espacio según el número de cubos
        std::mt19937                          rng{std::random_device{}()};
        std::uniform_real_distribution<float> distX(-12.0f, 12.0f);
        std::uniform_real_distribution<float> distZ(-12.0f, 12.0f);
        const float                           yFloor = -3.0f;

        for (int i = 0; i < MaximumSmallCubes; ++i)
        {
            float x = distX(rng);
            float z = distZ(rng);
            smallCubePositions.emplace_back(x, yFloor, z); // Añadir la posición de los cubos
        }
    }

    // Crear o actualizar el Top-Level Acceleration Structure (TLAS)
    if (!m_pTLAS)
    {
    ...
    // Setup instances
    // Aquí ya puedes utilizar el vector dinámico `Instances`
    ...
    Instances[1].Transform.SetTranslation(-10.0f, -3.0f, -5.0f);
    ...
    Instances[2].Transform.SetTranslation(0.0f, -3.0f, -5.0f);
    ...
    Instances[3].Transform.SetTranslation(10.0f, -3.0f, -5.0f);

    static std::string smallSphereNames[MaximumSmallSpheres];
    static std::string smallCubesNames[MaximumSmallCubes];
    ...
    for (int i = 0; i < MaximumSmallSpheres; ++i)
    {
        ...
        smallSphereNames[i] = "NearSphere" + std::to_string(i);
        ...
        inst.Transform.SetTranslation(pos.x, -3.0f, pos.z);
        inst.Transform.SetRotation(float3x3::Scale(0.5f, 0.5f, 0.5f).Data()); // Escala pequeña
    ...
    Attribs.pInstances    = Instances.data(); // Cambiar a `Instances.data()`
    Attribs.InstanceCount = Instances.size(); // Cambiar a `Instances.size()`
...
void Tutorial21_RayTracing::CreateSBT()
{
    ...
    struct InstanceHitGroup
    {
        const char* InstanceName;
        const char* PrimaryHit;
        const char* ShadowHit;
    } hitGroups[] = {
        {"Ground Instance", "GroundHit", nullptr},
        {"Cube Instance", "GlassPrimaryHit", nullptr},
        {"Cube Instance 2", "GlassPrimaryHit", nullptr},
        {"Cube Instance 3", "CubePrimaryHit", nullptr}};

    for (const auto& group : hitGroups)
    {
        if (group.PrimaryHit)
            m_pSBT->BindHitGroupForInstance(m_pTLAS, group.InstanceName, PRIMARY_RAY_INDEX, group.PrimaryHit);
        if (group.ShadowHit)
            m_pSBT->BindHitGroupForInstance(m_pTLAS, group.InstanceName, SHADOW_RAY_INDEX, group.ShadowHit);
    }

    constexpr int NumSpheres = MaximumSmallSpheres;
    for (int i = 0; i < NumSpheres; ++i)
    {
        char instanceName[32];
        std::snprintf(instanceName, sizeof(instanceName), "NearSphere%d", i);
...
void Tutorial21_RayTracing::UpdateUI()
{
    ...
    if (ImGui::Begin("Settings", nullptr, ImGuiWindowFlags_AlwaysAutoResize))
    {
        ...
        ImGui::Separator();
        ImGui::Text("# of spheres");
        if (ImGui::SliderInt("Count", &m_NumScatterSpheres, 0, MaximumSmallSpheres))
        {
            ...
            for (int i = 0; i < m_NumScatterSpheres; ++i)
            {
                ...
                m_ScatterSphereNames.push_back("Number of spheres" + std::to_string(i));
            }
        }
                ...
                ImGui::Separator();
        ImGui::Text("# of cubes");
        if (ImGui::SliderInt("Count", &m_NumScatterCubes, 0, MaximumSmallCubes))
        {
            // The user changed Count � rebuild the positions+names:
            ...
            {
                m_ScatterSpherePositions.emplace_back(distX(m_Rng), yFloor, distZ(m_Rng));
                m_ScatterSphereNames.push_back("Number of cubes" + std::to_string(i));
            }
        }
        ImGui::End();
    }
